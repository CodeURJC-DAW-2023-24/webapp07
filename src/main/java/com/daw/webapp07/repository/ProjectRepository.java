...
public interface ProjectRepository extends JpaRepository<Project, Long> {
    ...
    DROP VIEW IF EXISTS UProjectRelation;
    	...
    	CREATE VIEW UCategoryPercentage
            AS
                SELECT
    			uci.
            user_entity_id,
                uci.category,
    			uci.total_investment / u.total_investment
            AS
            percentage
    		FROM
                UCategoryInvestment uci
    				JO
              			(SELECT
    				user_entity_id, SUM(total_investment) AS
                total_investment
    			FROM
                UCategoryInvestment
    			GROUP BY user_entity_id) u ON u.user_entity_id = uci.
            user_entity_id
    		GROUP BY uci.
                user_entity_id , uci.category;
    		
    	CREATE VIEW
                UCategoryDifference AS
    		SELECT
    			u1.
            user_entity_id AS user1,
    			u2
            .user_entity_id AS user2,
    			u1.category,
    			ABS(u1.percentage
            - u2.
            percentage) AS differe
               		FRO
    ...
    CREATE VIEW
        ...
        SELECT
            ...
            COALESCE(
            SUM((us.
            total_difference * upds
            .total_donation)) / SUM(upds
            .total_donation), 200) AS relation_number
        FROM
            user_entity u
            CROSS JOIN Project p
            LEFT JOIN
            UProjectDonationSum upds ON upds.project = p.id
